...
def evaluate(instance_data, n_instance, problem_size, eva: callable):
    ...
    # --------- compute average cost1 and cost2 ----------
    for n_ins, objs in enumerate(Archives):
        if len(objs) > 0:
            avg_objs = np.mean(objs, axis=0)  # [avg_cost1, avg_cost2]
            obj_1[n_ins] = avg_objs[0]
            obj_2[n_ins] = avg_objs[1]
        else:
            obj_1[n_ins] = float('inf')
            obj_2[n_ins] = float('inf')
        ...
        if k == 1:
            for _ in range(1):
                ...
                if isinstance(result, str) and result.startswith("Error"):
                    ...
                    continue
                # result[0] = result[0] / (35*35)  # Comment out normalization vì giờ là cost thô, bạn có thể điều chỉnh nếu cần
                data[k]["score"] = result 
