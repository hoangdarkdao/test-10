...
def evaluate(instance_data, n_instance, ref_point, capacity, evaluate_func: callable):
    ...
    for n_ins, objs in enumerate(all_objs):
        if len(objs) > 0:
            obj_1[n_ins] = np.min(objs[:, 0])  # Min total_distance (raw cost 1)
            obj_2[n_ins] = np.min(objs[:, 1])  # Min longest_route (raw cost 2)
...
if __name__ == '__main__':
    ...
    for _ in range(5):
        cst1, cst2 = tsp.evaluate_program('_',select_neighbor)
        print("Total Distance (avg min):", cst1)
        print("Longest Route (avg min):", cst2)
        print("--------------------------------------------------")